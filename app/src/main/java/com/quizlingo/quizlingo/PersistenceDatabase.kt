...
class PersistenceDatabase(context: Context): AllDecksGetter, DeckInfoGetter, DeckSaver {
    ...
    private val database = DBImpl.AppDatabase.getDatabase(context)
    private val cardsDao = database.cardsDao()
    private val decksDao = database.decksDao()

    private class DBImpl {
        ...
        interface CardsDao {
            ...
            @Insert(onConflict =  OnConflictStrategy.REPLACE)
            suspend fun insertMany(card: List<Card>): Long
    ...
    override suspend fun getAllDecks(): List<Deck> {
        return decksDao.getDecks().map{ dbDeck ->
            val cards = cardsDao.getCardsByDeck(dbDeck.id).map{ dbCard ->
                Card(dbCard.id, dbCard.deckId, dbCard.cardPrompt, dbCard.cardText)
            }
            Deck(
                dbDeck.id,
                dbDeck.deckTitle,
    ...
    override suspend fun saveDeck(deck: Deck) {
        val dbDeck = DBImpl.Deck(deck.deckId, deck.title, deck.description, deck.cardCount)
        ...
        val dbCards = deck.cards.map{card ->
            DBImpl.Card(card.id, deckId, card.prompt, card.answer)
        }
    ...
    override suspend fun deleteDeck(deck: Deck) {
        val dbDeck = DBImpl.Deck(deck.deckId, deck.title, deck.description, deck.cardCount)
        val dbCards = deck.cards.map{card ->
            DBImpl.Card(card.id, card.deckId, card.prompt, card.answer)
        }
